...
mod test {
    ...
    fn conv_and_bias() {
        ...
        graph.new_tensor_f32("X", Some(vec![1.0, 2.0, 3.0, 4.0]), vec![1, 1, 2, 2]);
        graph.new_tensor_f32("W", Some(vec![0.1, 0.2, 0.3, 0.4]), vec![1, 1, 2, 2]);
        graph.new_tensor_f32("b", Some(vec![0.5]), vec![1]);
        graph.new_tensor_f32("Y", None, vec![1, 1, 3, 3]);
        graph
            .new_op(
                vec!["X", "W", "b"],
                vec!["Y"],
    ...
    fn conv_and_bias_larger_input() {
        ...
        graph.new_tensor_f32("b", Some(vec![0.5, 0.5]), vec![2]);
        graph.new_tensor_f32("Y", None, vec![1, 2, 7, 7]);
        graph
            .new_op(
                vec!["X", "W", "b"],
                vec!["Y"],
